...
def duplicateBitsOfList(l):
    return list(map(duplicateBits,l))

lim = pow(2,64)
P = list(map(duplicateBits,list(PI_P_ARRAY)))
S = list(map(duplicateBitsOfList,(list(map(list,PI_S_BOXES)))))

def f(x):
    return ((((S[0][x >> 56] + S[1][(x << 16) >> 40 & 0xff]) % lim) ^ S[2][(x << 32) >> 24 & 0xff]) + S[3][(x << 48) >> 8 & 0xff]) % lim 
